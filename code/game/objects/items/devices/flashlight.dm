...
/obj/item/device/flashlight/attack(mob/living/carbon/human/M, mob/living/carbon/human/user)
	...
	if(on && user.zone_sel.selecting == "eyes")
		...
		if(istype(M, /mob/living/carbon/human) && ((H.head && H.head.flags_cover & HEADCOVERSEYES) || (H.wear_mask && H.wear_mask.flags_cover & MASKCOVERSEYES) || (H.glasses && H.glasses.flags_cover & GLASSESCOVERSEYES)))
			user << "<span class='notice'>You need to remove [(H.head && H.head.flags & HEADCOVERSEYES) ? "that helmet" : (H.wear_mask && H.wear_mask.flags & MASKCOVERSEYES) ? "that mask": "those glasses"] first.</span>"
			return
...
/obj/item/device/flashlight/flare
	name = "flare"
	desc = "A red Vault-tec issued flare. There are instructions on the side, it reads 'pull cord, make light'."
	w_class = 2
