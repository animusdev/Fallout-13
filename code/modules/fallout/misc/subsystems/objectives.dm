...
//			give_points(mind, O.points)
			text += "\t<b>[faction.full_name]</b> <font color='#00FF00'>has succeeded to complete</font> [O.name] objective:<br>"
			text += "\t\t <i>[FormatText(desc, faction.objective.data)]</i><br>"
		else
			text += "\t<b>[faction.full_name]</b> <font color='#FF0000'>has failed to complete</font> [O.name] objective:<br>"
			text += "\t\t <i>[FormatText(desc, faction.objective.data)]</i><br>"
	...
	for(var/datum/mind/mind in ticker.minds)
		...
		if(O.check_complete(mind.objective))
			give_points(mind, O.points)
			text += "\t<b>[mind]</b> <font color='#00FF00'>has succeeded to complete</font> [O.name] objective:<br>"
