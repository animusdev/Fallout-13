	...
	for(var/area/A in world)
		if(A.outdoors && !stormactive)
			for(var/mob/M in view(10,A))
				...
				spawn(100)
					to_chat(M,"The clouds on the horizon get darker... A sandstorm storm is coming! You'd better find a safe shelter now.")
					spawn(600)
	...
	if(A.outdoors)
		...
		for(T in view(1,A))
			...
			for(M in view(7,T))
				...
				for(M in view(1,T))
					if(M.ckey)
						to_chat(M,"You have been hit by lightning! Your heart stopped.")
					M.death()
				spawn(1)
					return
